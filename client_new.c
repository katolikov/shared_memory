...
int main(){
     ...
     // C помощью этого системного вызова можно запрашивать и изменять управляющие параметры набора семафоров с дескриптором semfd (1),
     // а также удалять семафор. Значение num (2) — это индекс семафора, cmd (3)— задает операцию,
     // которая должна быть выполнена над конкретным семафором данного набора:
     // (копировать управляющие параметры набора семафоров в объект, указанный аргументом arg.buf.)
     // arg (4) — это объект типа union semun, который может использоваться для задания или
     // выборки управляющих параметров набора семафоров в соответствии с аргументом cmd.
